...
int main(int argc, char* argv[]) {
    ...
    if (options.ip == "127.0.0.1") {
        LOKI_LOG(critical,
                 "Tried to bind data-storage to localhost, please bind "
    ...
    LOKI_LOG(info, "Setting Bittorod RPC to {}:{}", options.lokid_rpc_ip, options.lokid_rpc_port);
    ...
    try {
        ...
        LOKI_LOG(info, "BITTOROD LEGACY KEY: {}", options.lokid_key);
        ...
        LOKI_LOG(info, "Retrieved keys from Bittorod; our SN pubkey is: {}",

...
int main(int argc, char* argv[]) {
    ...
    if (options.ip == "127.0.0.1") {
        LOKI_LOG(critical, "Tried to bind data-storage to localhost, please bind "
    ...
    LOKI_LOG(info, "Setting bittorod key path to {}", options.lokid_key_path);
    LOKI_LOG(info, "Setting bittorod RPC port to {}", options.lokid_rpc_port);
